# openworkflow · 本地/弱模型集成说明(Local-LLM Integration Notes)

> **状态:硬化已完成(P1–P7 全部内化)。** 本文已从「硬化指南」**降级为集成 + 设计说明**。
> openworkflow 现在**无需任何外部补丁**即可在 xclaude(rebrand 的 Claude Code + 本地 Qwen)/ 任意本地弱模型下完整驱动:script 摄取容错、教学式错误、`task=` 自动设计、命名模板、`OPENWORKFLOW_LENIENT` 开关全部是仓库内一等公民,有测试、有 CI。
>
> 还想用本文做什么:① 按「§1 集成配方」把 openworkflow 接进 xc(纯配置);② 「§5 验收」是回归基线;③ 「§4」保留为「做了什么/在哪」的索引;④ 「§8 设计边界」说明部署形态的固有限制。**§3/§6 仅留作历史对照。**
...
F1/F2(及 P1 路线图列出的全部失效模式)已从「外部补丁」升级为仓库内一等公民(默认开、有测试、可经 `OPENWORKFLOW_LENIENT=0` 关回严格)。**已无独立的外部补丁**;§6 仅留作历史对照。

---

## 4. 已内化的硬化项(做了什么 / 在哪)

> 下表原为路线图,现全部完成,保留作索引。每项:**问题 → 做法 → 验收 → 已实现位置**。所有用例在 `tests/test_weak_model_inputs.py`、`tests/test_templates.py`、`tests/test_mcp_server.py`。
...
- [x] **P7 · 文档回写**✅
  ...
  - **已实现**:`README.md` / `README.zh-CN.md` 新增「Local / weak-model use」小节(`task=`、命名模板、容错、`OPENWORKFLOW_LENIENT`、mcp.json 配方);本文 checklist 全勾;§6「当前外部补丁」框架降级为历史对照;标题/引言改为「集成说明」。
...
3. ✅ **命名 workflow**:`WorkflowList` 可列、`name=` 可调内置模板(`code-review` / `vote` / `deep-research` / `bug-hunt`);
4. ✅ **零外部补丁**:容错全部在仓库内(`mcp_server.py` / `sandbox.py`),xc 侧只剩**配置**(mcp.json env + settings allow),无代码注入。

1–4 已全部达成 → 本文「外部补丁」框架(§6)已降级为历史对照,本文转为「集成 + 设计说明」。回归基线:`pytest` **125 绿**(含弱样本 + 端到端)。

---

## 6. 历史对照:曾经的外部补丁(已被内化取代)

> 仅作历史记录。下面是 P1 之前唯一的「外部补丁」(`_unwrap_script` + `_nullish` + `_sanitize_args`,塞在 `execute_workflow` 入口)。它只覆盖「引号/JSON 包裹」与「`"null"`」两种失效模式。

它**已被**仓库内的一等公民取代:

| 旧补丁 | 现在的实现 | 增量 |
|---|---|---|
| `_unwrap_script`(剥引号 / `json.loads`) | `sandbox.normalize_script`(`tests/test_weak_model_inputs.py`) | 增加 md 围栏、单/三引号、前导散文、转义换行、任意组合;`ast.parse` + 首句 `meta` 结构校验;干净脚本零改动 |
| `_nullish` | `mcp_server._nullish`(纳入测试) | 不变,但挂上 `OPENWORKFLOW_LENIENT` 开关 |
| `_sanitize_args`(裸调用) | `mcp_server._sanitize_args`(经 `_lenient()` 门控) | 增加「源码误塞 `name`/`scriptPath` 时重路由」 |

要查看现行实现:`openworkflow/sandbox.py::normalize_script` 与 `openworkflow/mcp_server.py::_sanitize_args`。要恢复「严格原生契约」:`OPENWORKFLOW_LENIENT=0`。
...
```bash
# 0) 容错回归(不花 GPU,最快):跑全套测试,含弱样本 + 端到端
pytest                                                       # 应 125 passed
...
openworkflow do "并行回答3个常识问题再汇总" --backend tool                     # P3:task= 的 CLI 等价(design→run)
